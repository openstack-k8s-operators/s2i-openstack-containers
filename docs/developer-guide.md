...
`cinderVolumeImages` / `manilaShareImages` are not in the scalar map.
`s2i-test-base` synthesizes those backend maps from the provider
registry and tag **only** when `cinder/cinder-volume` or
`manila/manila-share` is in `s2i_ci_content.selected_images`. The same
gate applies to `cifmw_test_operator_tempest_image` on
`s2i-openstack-deploy-validation`. The tempest fallback uses
`cifmw_default_*` group_vars (`quay.io` /
`podified-antelope-centos9` / `openstack-tempest-all` /
`current-podified`), not `cifmw_test_operator_default_*` role
defaults, because those are undefined when `cifmw_setup` dumps
`ci_framework_params` at bootstrap. A service-only `changes` build
must not point volume, share, or tempest pods at a tag the registry
does not have.
