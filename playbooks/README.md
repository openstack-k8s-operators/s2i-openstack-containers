...
| `playbooks/install-build-deps.yaml` | Install `buildah`/`podman` on the job node |
| `playbooks/pre.yaml` | Stage Zuul checkouts into `containers/.../src/...` |
| `playbooks/run.yaml` | Resolve repo dir + invoke `s2i_build` |
| `zuul.d/jobs.yaml` | Job definitions (parent: `base` / `tox`) |
...
### 3. Optional: drive a job from another project

Define a child job that parents `s2i-openstack-containers-build-watcher`
(or the base) and put `Depends-On: <this-PR-URL>` in the change description.
The s2i repo must already be in the RDO tenant for Depends-On /
required-projects to resolve.
